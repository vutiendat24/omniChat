...
| M13 — User Service | `user-service` | M00 | ✅ MOD-USR-01, MOD-USR-02, MOD-USR-03, MOD-USR-04 done |
...
| `conversation-service` | MySQL | `conversations` (OPEN/RESOLVED/PENDING/SPAM, sla_due_at, sla_paused_at, first_responded_at, last_message_at, customer_name, customer_phone, customer_avatar), `conversation_history`, `messages` (message_type, payload JSON, is_deleted, INBOUND/OUTBOUND, SENDING/DELIVERED), `quick_replies`, `tags`, `private_reply_records` | Idempotency key (message_id); Distributed Lock Redis; Batch insert cho livestream |
